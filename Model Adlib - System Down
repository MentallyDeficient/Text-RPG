...
import textwrap
import random
from fabric import Connection   # For the terminal room. Connection will be used in the OldTerminal class to remote to Potato
...
class Room:
    ...
    # 1.2 Modification, I added first_visit variable to Rooms init method and set it to true. So the player gets the long verse description
    # the first time the go to a new room and the short and sweet for every subsequent visit. @when(look) got modified to read True and False
    def __init__(self, description, sub_description, first_visit=True):
        self.description = description.strip()
        self.sub_description = sub_description.strip()
        self.first_visit = first_visit
        ### This needs to replace the regular description for look() and needs to be written on What.py ###
